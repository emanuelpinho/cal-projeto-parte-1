...
class Vertex {
	...
	/*
	 * Guardar a cobertura a actual de cada vértice
	 * e a existência de armazém e posto de vigília
	 */
	int coverage;
	bool warehouse;
	bool vigilance;
...
public:
	...
	bool hasWarehouse();
	void setWarehouse(bool wh);

	bool hasVigilance();
	void setVigilance(bool v);
...
template<class T>
void Vertex<T>::incrementCoverage() {
	coverage++;
}

template<class T>
int Vertex<T>::getCoverage() {
	return coverage;
}

template<class T>
bool Vertex<T>::hasWarehouse() {
	return warehouse;
}

template<class T>
void Vertex<T>::setWarehouse(bool wh) {
	warehouse = wh;
}

template<class T>
bool Vertex<T>::hasVigilance() {
	return vigilance;
}

template<class T>
void Vertex<T>::setVigilance(bool v) {
	vigilance = v;
}
...
public:
	bool addVertex(const T &in, bool wh);
	...
	/*
	 * Calcula a cobertura mínima que os postos terão de ter
	 */
	int getMinCoverage() const;
	...
	bool areVertexCovered();

};

template<class T>
bool Graph<T>::areVertexCovered() {
	bool r = true;
...
template<class T>
void Graph<T>::placeVigilance() {
	while (!areVertexCovered()) {
		Vertex<T> *chosenOne;
		int coveredVertex = 0;

		for (unsigned int c = 0; c < vertexSet.size(); c++) {

			Vertex<T> *vOrigin = vertexSet[c];
			if (vOrigin->hasVigilance())
				continue;
			// armazena o numero de vértices que consegue este vOrigin cobrir
			int coveredVertexTemp = 0;

			dijkstraShortestPath(vOrigin->getInfo());

			for (unsigned int i = 0; i < vertexSet.size(); i++) {
				Vertex<T> *vDest = vertexSet[i];
				double distance = vDest->getDist();

				if (distance <= coverage) {

					if (vDest->hasWarehouse()) {
						if (vDest->getCoverage() < 2) {
							coveredVertexTemp++;
						}
					} else {
						if (vDest->getCoverage() < 1)
							coveredVertexTemp++;
					}
			...
			if (coveredVertexTemp > coveredVertex) {
				chosenOne = vOrigin;
				coveredVertex = coveredVertexTemp;
			}
			else if(coveredVertexTemp == coveredVertex){

			}
		}

		chosenOne->setVigilance(true);
		chosenOne->incrementCoverage();
		dijkstraShortestPath(chosenOne->getInfo());
		for (unsigned int i = 0; i < vertexSet.size(); i++) {
			Vertex<T> *v = vertexSet[i];
			double distance = v->getDist();
			if (distance <= coverage) {
				v->incrementCoverage();
			}
		}
...
template<class T>
int Graph<T>::getMinCoverage() const {

	vector<double> minEdges;

	for (unsigned int c = 0; c < vertexSet.size(); c++) {

		if (vertexSet[c]->hasWarehouse()) {

			vector<Edge<T> > adj = vertexSet[c]->getEdges();
			if (adj.size() == 0)
				return -1;

			int weight = adj[0].getWeight();
			int tempWeight;

			for (unsigned int i = 1; i < adj.size(); i++) {
				tempWeight = adj[i].getWeight();
				if (tempWeight < weight)
					weight = tempWeight;
			}
			minEdges.push_back(weight);
		}
	}
	...
	for (unsigned int c = 1; c < minEdges.size(); c++) {
		if (minEdges[c] > val)
			val = minEdges[c];
	}
...
template<class T>
bool Graph<T>::addVertex(const T &in, bool wh) {
...
void Graph<T>::extractToViewer() {
	...
	for (unsigned int i = 0; i < vertexSet.size(); i++) {
		...
		gv->addNode(c);
		if (vertexSet[i]->hasWarehouse()) {
			gv->setVertexColor(vertexSet[i]->getInfo(), "green");
		}
	}
	...
	for (unsigned int i = 0; i < vertexSet.size(); i++) {
		...
		for (unsigned int c = 0; c < vertexSet[i]->getEdges().size(); c++) {
			destino = vertexSet[i]->getEdges()[c].getDest()->getInfo();
			gv->addEdge(arestas, origem, destino, EdgeType::DIRECTED);

			double weight = vertexSet[i]->getEdges()[c].getWeight();

			stringstream s;
			s << weight;
			gv->setEdgeLabel(arestas, s.str());
			arestas++;
		}
...
void Graph<T>::extractFinal() {
	GraphViewer *gv = new GraphViewer(600, 600, true, 7773);
	gv->createWindow(600, 600);
	gv->defineEdgeColor("blue");
	gv->defineVertexColor("lightGray");
	...
	for (unsigned int i = 0; i < vertexSet.size(); i++) {
		...
		if (vertexSet[i]->hasWarehouse()) {
			if(vertexSet[i]->hasVigilance())
				gv->setVertexColor(vertexSet[i]->getInfo(), "pink");
			else
				gv->setVertexColor(vertexSet[i]->getInfo(), "red");

		} else {
			if (vertexSet[i]->hasVigilance()) {
				gv->setVertexColor(vertexSet[i]->getInfo(), "blue");
			}
		}
